...
namespace JobEventBusRabbitMq
{
    public class JobEventBusRabbitMq : IJobEventBus, IDisposable, IHostedService
    {
            ...
            IServiceProvider serviceProvider)
        {
            ...
            // 客户端名称(或是服务名),用来区分不同服务的routingKey
            _clientName = options.ClientProvidedName + ".";
        }
        ...
        public void Publish(JobIntegrationEvent @event)
        {
            ...
            using (var channel = _connection.CreateModel())
            {
                ...
                policy.Execute(() =>
                {
                    ...
                    // 不同服务的routingKey不同,由ClientProvidedName配置区别
                    channel.BasicPublish(exchange: _exchangeName, routingKey: _clientName + eventName, mandatory: true, basicProperties: properties, body: body);
                });
            }
        }
        ...
        {
            var eventName = eventArgs.RoutingKey.Replace(_clientName, "");
                ...
                try
                {
                    ...
                    foreach (var (eventName, _) in _jobSubscriptionInfo.EventHandlerTypes)
                    {
                        ...
                        _consumerChannel.QueueBind(
                            ...
                            routingKey: _clientName + eventName);
                    }
                }
                catch (Exception ex)
                {
